...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('assets.store') }}" method="POST">
            ...
            <div class="form-group"><label for="type">Type *</label><select class="form-control" id="type" name="type" required><option value="">Select</option><option value="laptop" {{ old('type') == 'laptop' ? 'selected' : '' }}>Laptop</option><option value="phone" {{ old('type') == 'phone' ? 'selected' : '' }}>Phone</option><option value="monitor" {{ old('type') == 'monitor' ? 'selected' : '' }}>Monitor</option><option value="other" {{ old('type') == 'other' ? 'selected' : '' }}>Other</option></select></div>
            ...
            <div class="form-group"><label for="employee_id">Assign to (optional)</label><select class="form-control" id="employee_id" name="employee_id"><option value="">— None —</option>@foreach($employees as $emp)<option value="{{ $emp->id }}" {{ old('employee_id') == $emp->id ? 'selected' : '' }}>{{ $emp->full_name }}</option>@endforeach</select></div>
